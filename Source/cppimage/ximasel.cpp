...
namespace CppImage
{
	...
	bool CxImage::SelectionAddEllipse(Rect r, uint8_t level)
	{
		...
		if (info.rSelectionBox.left > (xcenter - xradius)) info.rSelectionBox.left = max(0,min(head.biWidth,(xcenter - xradius)));
		if (info.rSelectionBox.right <= (xcenter + xradius)) info.rSelectionBox.right = max(0,min(head.biWidth,(xcenter + xradius + 1)));
		if (info.rSelectionBox.bottom > (ycenter - yradius)) info.rSelectionBox.bottom = max(0,min(head.biHeight,(ycenter - yradius)));
		if (info.rSelectionBox.top <= (ycenter + yradius)) info.rSelectionBox.top = max(0,min(head.biHeight,(ycenter + yradius + 1)));

		int32_t xmin = max(0, min(head.biWidth,xcenter - xradius));
		int32_t xmax = max(0, min(head.biWidth,xcenter + xradius + 1));
		int32_t ymin = max(0, min(head.biHeight,ycenter - yradius));
		int32_t ymax = max(0, min(head.biHeight,ycenter + yradius + 1));
	...
	bool CxImage::SelectionAddPolygon(Point *points, int32_t npoints, uint8_t level)
	{
		...
		while (i < npoints){
			...
			if (localbox.top < r2.top) localbox.top = max(0,min(head.biHeight-1,r2.top+1));
			if (localbox.left > r2.left) localbox.left = max(0,min(head.biWidth-1,r2.left-1));
			if (localbox.right < r2.right) localbox.right = max(0,min(head.biWidth-1,r2.right+1));
			if (localbox.bottom > r2.bottom) localbox.bottom = max(0,min(head.biHeight-1,r2.bottom-1));
