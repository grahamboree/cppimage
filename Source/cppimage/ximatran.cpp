...
namespace CppImage
{
		...
		for (int32_t y=0;y<head.biHeight;y++){
			...
			for (int32_t x=0;x<head.biWidth;x++){
				if (!errordiffusion){
					...
					c.rgbRed = (uint8_t)min(255,max(0,(int32_t)c.rgbRed + ((er*7)/16)));
					c.rgbGreen = (uint8_t)min(255,max(0,(int32_t)c.rgbGreen + ((eg*7)/16)));
					c.rgbBlue = (uint8_t)min(255,max(0,(int32_t)c.rgbBlue + ((eb*7)/16)));
					...
					for(int32_t i=-1; i<2; i++){
						...
						c.rgbRed = (uint8_t)min(255,max(0,(int32_t)c.rgbRed + ((er * coeff)/16)));
						c.rgbGreen = (uint8_t)min(255,max(0,(int32_t)c.rgbGreen + ((eg * coeff)/16)));
						c.rgbBlue = (uint8_t)min(255,max(0,(int32_t)c.rgbBlue + ((eb * coeff)/16)));
	...
	bool CxImage::Crop(int32_t left, int32_t top, int32_t right, int32_t bottom, CxImage* iDst)
	{
		...
		int32_t startx = max(0,min(left,head.biWidth));
		int32_t endx = max(0,min(right,head.biWidth));
		int32_t starty = head.biHeight - max(0, min(top,head.biHeight));
		int32_t endy = head.biHeight - max(0, min(bottom,head.biHeight));
