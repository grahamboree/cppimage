...
namespace CppImage
{
	...
	public:
		inline CxImageICO(): CxImage(CXIMAGE_FORMAT_ICO) , m_dwImageOffset(0) {}
	...
	//	bool Save(const TCHAR * imageFileName){ return CxImage::Save(imageFileName,CXIMAGE_FORMAT_ICO);}
		...
		inline bool Decode(FILE *hFile) { CxIOFile file(hFile); return Decode(&file); }
		...
		bool Encode(FILE *hFile, bool bAppend=false, int32_t nPageCount=0) { CxIOFile file(hFile); return Encode(&file,bAppend,nPageCount); }
		bool Encode(FILE *hFile, CxImage ** pImages, int32_t nPageCount) { CxIOFile file(hFile); return Encode(&file, pImages, nPageCount); }
