...
namespace CppImage
{
					   ...
					   int32_t direction, bool bForceFFT, bool bMagnitude)
	{
		...
		for (j=0;j<h;j++) {
			for (k=0;k<w;k++) {
				if (bMagnitude){
					tmpReal->SetPixelIndex(k,j,(uint8_t)max(0.0,min(255.0,(nn*(3+log(_cabs(grid[k][j])))))));
					if (grid[k][j].x==0){
						tmpImag->SetPixelIndex(k,j,(uint8_t)max(0.0,min(255.0,(128+(atan(grid[k][j].y/0.0000000001)*nn)))));
					} else {
						tmpImag->SetPixelIndex(k,j,(uint8_t)max(0.0,min(255.0,(128+(atan(grid[k][j].y/grid[k][j].x)*nn)))));
					}
				} else {
					tmpReal->SetPixelIndex(k,j,(uint8_t)max(0.0,min(255.0,(128 + grid[k][j].x*nn))));
					tmpImag->SetPixelIndex(k,j,(uint8_t)max(0.0,min(255.0,(128 + grid[k][j].y*nn))));
				}
			}
		}
	...
	bool CxImage::RepairChannel(CxImage *ch, float radius)
	{
		...
		for(x=1; x<w; x++){
			for(y=1; y<h; y++){
				...
				tmp.BlindSetPixelIndex(x,y,(uint8_t)min(255.0,max(0.0,(xy0 + radius * correction + 0.5))));
			}
		}
	...
	bool CxImage::AdaptiveThreshold(int32_t method, int32_t nBoxSize, CxImage* pContrastMask, int32_t nBias, float fGlobalLocalBalance)
	{
		...
		for (int32_t y=0; y<mh; y++){
			for (int32_t x=0; x<mw; x++){
				...
				mask.SetPixelIndex(x,y,(uint8_t)max(0.0f, min(255.0f,nBias+((1.0f-fGlobalLocalBalance)*threshold + fGlobalLocalBalance*globalthreshold))));
			}
		}
