...
namespace CppImage
{
	...
	{
		return static_cast<tsize_t>(static_cast<CxFile*>(fd)->Read(buf, 1, static_cast<size_t>(size)));
	}

	//////////////////////////////////////////////////////////////////////////
	static tsize_t _tiffWriteProcEx(thandle_t fd, tdata_t buf, tsize_t size)
	{
		return static_cast<tsize_t>(static_cast<CxFile*>(fd)->Write(buf, 1, static_cast<size_t>(size)));
	}
	...
	static toff_t _tiffSeekProcEx(thandle_t fd, toff_t off, int whence)
	{
		if ( off == 0xFFFFFFFF ) 
			return 0xFFFFFFFF;
		if (!(static_cast<CxFile*>(fd))->Seek(static_cast<int32_t>(off), whence))
			return 0xFFFFFFFF;
		if (whence == SEEK_SET)
			return off;
